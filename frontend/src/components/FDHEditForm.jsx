...
export default function FDHEditForm({ fdh, onSuccess, onClose }) {
  const [formData, setFormData] = useState({
    name: fdh.name || '',
    location: fdh.location || '',
    region: fdh.region || '',
    max_ports: fdh.max_ports || 128,
  });
  ...
  const handleSubmit = (e) => {
    ...
    const updatePayload = {
      ...formData,
      max_ports: parseInt(formData.max_ports),
    };

    fetch(`/api/network-hierarchy/fdhs/${fdh.fdh_id}`, {
      method: 'PUT',
      ...
      .then((res) => {
        if (!res.ok) {
          return res.json().then(err => { throw new Error(err.detail || 'Failed to update FDH') });
        }
        return res.json();
      })
      .then(() => {
